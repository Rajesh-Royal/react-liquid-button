    ...
    <>
        ...
        <DrawerContent className='sidebar-mobile'>
          ...
          <DrawerBody pb={'6em'}>
            ...
            <Flex direction="column">
              <Link to="https://github.com/Rajesh-Royal/react-liquid-button" target='_blank' display="block" mb={2} onClick={() => setDrawerOpen(false)}>
                GitHub<ArrowForwardIcon boxSize={7} transform={"rotate(-45deg)"} position="relative" top="-1px" />
              </Link>
              <Link to="https://portfolio-rajesh-royal.vercel.app/" target='_blank' display="block" mb={2} onClick={() => setDrawerOpen(false)}>
      ...
      <Box as="nav" position="fixed" top={'57px'} height="calc(100vh - 57px)" className='sidebar' overflowY="auto" bg={sidebarBgColor} w={{ base: 0, md: 60 }} p={5} display={{ base: 'none', md: 'block' }}>
        <VStack align="stretch" spacing={4}>
          {CATEGORIES.map(category => (
            <Category key={category.name} category={category} location={location} handleClick={() => scrollToTop()} hoverColor={linkHoverColor} />
          ))}
        </VStack>
      </Box>
    </>
